...
export default class HomeScreen extends React.Component {
  constructor(props) {
    ...
    this.searchCardByName = this.searchCardByName.bind(this)
  }
          ...
          </View>
          {
            this.state.card.name
              ? <View>
                ...
                <Text>Price: {this.state.card.usd}</Text>
                <Image source={{ uri: `${this.state.card.image_uris.png}` }}
                  style={{ width: 250, height: 350 }} />
              </View>
              : <Text>{this.state.searchError}</Text>
          }
        </ScrollView>
  ...
  async searchCardByName() {
    try {
      const { data: card } = await axios.get(`https://api.scryfall.com/cards/named?fuzzy=${this.state.cardName}`)
      this.setState({ card })
    } catch (err) {
      this.setState({ searchError: err.toString() })
    }
  }
